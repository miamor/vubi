function del (itemID) {
    ...
    if (itemID) {
        if (confirm("Are you sure want to remove "+itemID+"'s services permanently?")) {
            /*var row = table.row($a.closest('tr'));
            var rowNode = row.node();
            row.remove();*/

            $.get(MAIN_URL+'/requests/services/delete.php?id='+itemID, function (data) {
                console.log(data);
...
$(document).ready(function () {
    $('#buyList').DataTable({
		"ajax": MAIN_URL+'/requests/services/readAll.php',
		"columns": [
            { 
                data: "id",
                render : function (data, type, row) {
                    return '<a href="'+MAIN_URL+'/services/'+row.id+'">'+data+'</a></div>'
                }
            },
            {
                data: "title",
